...
// Janela para reordenar e ativar/desativar tokens da linha de tarefa.
public partial class TaskRowOrderWindow : Window
{
    ...
    // Estado da ordem atual — lista mutável de (key, ativo)
    private readonly List<(string Key, bool Active)> _state = [];

    public TaskRowOrderWindow(IReadOnlyList<string> current, Action<List<string>> onSave)
    {
        InitializeComponent();
        _onSave = onSave;

        // Tokens ativos na ordem atual
        foreach (var key in current)
            _state.Add((key, true));

        // Tokens ausentes no final, desativados
    ...
    private void RenderList()
    {
        ...
        for (int i = 0; i < _state.Count; i++)
        {
            ...
            var idx = i; // captura para closure
            ...
            var up = MakeArrowBtn("▲", idx, -1);
            var dn = MakeArrowBtn("▼", idx, +1);
            btnPanel.Children.Add(up);
            btnPanel.Children.Add(dn);
